	...
	config = function()
		...
		local function setup_quarto_keymaps(bufnr)
			vim.keymap.set("n", "<leader>xc", function()
				runner.run_cell()
				set_search_term()
			end, { desc = "Quarto: run cell", silent = true, buffer = bufnr })
			...
			vim.keymap.set("n", "<leader>xb", function()
				...
				set_search_term()
				vim.cmd("normal ]b")
			end, { desc = "Quarto: run cell and goto next", silent = true, buffer = bufnr })
			...
			vim.keymap.set("n", "<leader>xu", function()
				runner.run_above()
				set_search_term()
			end, { desc = "Quarto: run cell and above", silent = true, buffer = bufnr })
			vim.keymap.set("n", "<leader>xa", function()
				runner.run_all()
				set_search_term()
			end, { desc = "Quarto: run all cells", silent = true, buffer = bufnr })
			vim.keymap.set("n", "<leader>xl", function()
				runner.run_line()
				set_search_term()
			end, { desc = "Quarto: run line", silent = true, buffer = bufnr })
			vim.keymap.set("n", "<leader>XA", function()
				runner.run_all(true)
				set_search_term()
			end, { desc = "Quarto: run all cells of all languages", silent = true, buffer = bufnr })
		end
