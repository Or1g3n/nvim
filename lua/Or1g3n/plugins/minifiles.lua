	...
	config = function()
		...
		vim.api.nvim_create_autocmd("User", {
			...
			callback = function(args)
				...
				end, { buffer = buf_id, desc = "Toggle file preview" })
				...
				map.set("n", "<Esc>", function()
					minifiles.close()
				end, { buffer = buf_id, desc = "MiniFiles: Close" })
				...
				local function toggle_edit_mode()
					...
					if edit_mode_active then
						...
						vim.keymap.set("n", "h", function()
							minifiles.go_out()
						end, { buffer = buf_id, desc = "MiniFiles: Left navigation" })
						vim.keymap.set("n", "l", function()
							minifiles.go_in()
						end, { buffer = buf_id, desc = "MiniFiles: Right navigation" })
				...
				vim.keymap.set(
					...
					"<Leader>ee",
					toggle_edit_mode,
					{ buffer = buf_id, desc = "MiniFiles: Toggle Edit Mode" }
				)
			end,
		})
		...
		map.set("n", "<Leader>e", function()
			...
			noremap = true,
			silent = true,
			desc = "MiniFiles: Open file explorer (file directory or cwd fallback)",
		})
		map.set("n", "<Leader>E", function()
			minifiles.open()
		end, { noremap = true, silent = true, desc = "MiniFiles: Open file explorer (cwd)" })
		...
		local function set_mark(id, path, desc)
			if vim.fn.isdirectory(vim.fn.expand(path)) ~= 0 then
				...
				vim.notify(
					"MiniFiles: Bookmark path is not valid **"
						.. path
						.. "**\n\n"
						.. "Please update **"
						.. string.gsub(
							vim.fn.stdpath("config") .. "/lua/Or1g3n/plugins/local/minifiles/bookmarks.lua",
							"\\",
							"/"
						)
						.. "**",
