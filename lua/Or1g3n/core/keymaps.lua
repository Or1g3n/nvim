...
vim.api.nvim_create_autocmd("FileType", {
	...
	callback = function(args)
		...
		vim.keymap.set("n", "<Leader>%", function()
			vim.cmd("source %")
			vim.notify("File has been sourced!", vim.log.levels.INFO, { id = "source_file" })
		end, { noremap = true, silent = true, desc = "Editor: source file", buffer = bufnr })

		vim.keymap.set("n", "<Leader>x", function()
			vim.cmd(".lua")
			vim.notify("Line has been executed!", vim.log.levels.INFO, { id = "execute_line" })
...
end, { noremap = true, silent = true, desc = "Editor: Toggle tabline (showtabline)" })
...
map.set(
	"n",
	"<Leader>ra",
	'yiw:%s/<C-r>"//gc<Left><Left><Left>',
	{ noremap = true, silent = false, desc = "Editor: Replace all occurrences of word" }
)
map.set(
	"v",
	"<Leader>ra",
	'y:%s/<C-r>"//gc<Left><Left><Left>',
	{ noremap = true, silent = false, desc = "Editor: Replace all occurrences of selection" }
)
map.set(
	"n",
	"<Leader>rl",
	'yiw:s/<C-r>"//gc<Left><Left><Left>',
	{ noremap = true, silent = false, desc = "Editor: Replace occurrences of word on line" }
)
map.set(
	"v",
	"<Leader>rl",
	'y:s/<C-r>"//gc<Left><Left><Left>',
	{ noremap = true, silent = false, desc = "Editor: Replace occurrences of selection on line" }
)
map.set(
	"n",
	"<Leader>n",
	'yiw:let @/ = "<C-r>""<CR>',
	{ noremap = true, silent = false, desc = "Editor: Search word under cursor" }
)
map.set(
	"v",
	"<Leader>n",
	'y:let @/ = "<C-r>""<CR>',
	{ noremap = true, silent = false, desc = "Editor: Search highlighted word" }
)
...
map.set(
	"n",
	"j", -- Improve nagivating wrapped line behavior
	function(...)
...
map.set(
	"n",
	"k", -- Improve nagivating wrapped line behavior
	function(...)
		local count = vim.v.count
		if count == 0 then
			return "gk"
...
map.set(
	...
	"<Leader>p",
	'"_dP',
	{ noremap = true, silent = true, desc = "Editor: Paste without overwriting clipboard" }
)
...
map.set(
	...
	"J",
	"mzJ`z",
	{ noremap = true, silent = true, desc = "Editor: Join lines while maintaining cursor position" }
)
...
end, { noremap = true, silent = true, desc = "Editor: Auto-indent entire file" })

-- Show messages history
map.set(
	"n",
	"<Leader>.",
	":new | put =execute('messages') | wincmd J | res -15 <CR>",
	{ noremap = true, silent = true, desc = "Messages: Open message history in bottom buffer" }
)
...
end, { expr = true, noremap = true, desc = "Cmd: Start completion or accept selection" })
...
end, { expr = true, noremap = true, desc = "Cmd: Start completion or cycle selection" })
...
end, { expr = true, noremap = true, desc = "Cmd: Cycle next completion or command history" })
...
end, { expr = true, noremap = true, desc = "Cmd: Cycle previous completion or command history" })
...
end, { expr = true, noremap = true, desc = "Cmd: Expand file completion or move left" })
...
end, { expr = true, noremap = true, desc = "Cmd: Expand file completion or move right" })
...
-- Git
map.set("n", "<A-g><A-c>", function()
	vim.cmd(":let @/ = '^<\\|^=\\|^>'")
end, { noremap = true, desc = "Git: set search to conflict markers" })
