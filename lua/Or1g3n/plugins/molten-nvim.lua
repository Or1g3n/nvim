...
local function show_kernel_vars(result)
	...
	-- Set nowrap for the floating window
	vim.api.nvim_set_option_value("wrap", false, { win = state.floating.win })
...
return {
	...
	init = function()
		...
		local function define_cell_block()
			...
			local cell_tag = cell_tags.non_markdown[filetype] and cell_tags.non_markdown[filetype].cell_start
				or "No match"
		...
		vim.api.nvim_create_autocmd("FileType", {
			...
			callback = function()
				vim.keymap.set(
					...
					"<A-r><A-i>",
					":MoltenInit<CR>",
					{ silent = true, buffer = true, desc = "Molten: Initialize plugin" }
				)
				vim.keymap.set(
					"n",
					"<A-r><A-t>",
					":MoltenDeinit<CR>",
					{ silent = true, buffer = true, desc = "Molten: De-initialize plugin" }
				)
				vim.keymap.set(
					"n",
					"<A-r><A-d>",
					":MoltenDelete<CR>",
					{ silent = true, buffer = true, desc = "Molten: Delete cell" }
				)
				vim.keymap.set(
					"n",
					"<A-r><A-e>",
					":MoltenEvaluateOperator<CR>",
					{ silent = true, buffer = true, desc = "Molten: Run operator selection" }
				)
				vim.keymap.set(
					"n",
					"<A-r><A-r>",
					":MoltenEvaluateLine<CR>",
					{ silent = true, buffer = true, desc = "Molten: Evaluate line" }
				)
				vim.keymap.set(
					"n",
					"<A-r><A-c>",
					":MoltenReevaluateCell<CR>",
					{ silent = true, buffer = true, desc = "Molten: Re-evaluate cell" }
				)
				vim.keymap.set(
					"n",
					"<A-r><A-a>",
					":MoltenReevaluateAll<CR>",
					{ silent = true, buffer = true, desc = "Molten: Re-evaluate all cells" }
				)
				vim.keymap.set(
					"v",
					"<A-r>",
					":<C-u>MoltenEvaluateVisual<CR>gv<Esc>",
					{ silent = true, buffer = true, desc = "Molten: Evaluate visual selection" }
				)
				vim.keymap.set(
					"v",
					"<C-CR>",
				...
				vim.keymap.set(
					...
					"<A-r><A-y>",
					":MoltenYankOutput<CR>",
					{ silent = true, buffer = true, desc = "Molten: Yank output" }
				)
				vim.keymap.set(
					"n",
					"<Leader><A-r><A-y>",
					":MoltenYankOutput!<CR>",
					{ silent = true, buffer = true, desc = "Molten: Yank output (system clipboard)" }
				)
				vim.keymap.set(
					"n",
					"<A-r><A-h>",
					":MoltenHideOutput<CR>",
					{ silent = true, buffer = false, desc = "Molten: Hide output" }
				)
				...
				vim.keymap.set(
					...
					"<A-r><A-n>",
					":MoltenNext<CR>",
					{ silent = true, buffer = true, desc = "Molten: Goto next cell" }
				)
				vim.keymap.set(
					"n",
					"<A-r><A-p>",
					":MoltenPrev<CR>",
					{ silent = true, buffer = true, desc = "Molten: Goto previous cell" }
				)
				vim.keymap.set(
					"n",
					"<A-r><A-v>",
					":MoltenToggleVirtual<CR>",
					{ silent = true, buffer = true, desc = "Molten: Toggle cell virt_text_output" }
				)
				vim.keymap.set(
					"n",
					"<Leader><A-r><A-v>",
					":MoltenToggleVirtual!<CR>",
					{ silent = true, buffer = true, desc = "Molten: Toggle all cells virt_text_output" }
				)
				vim.keymap.set("n", "<S-CR>", function()
					run_cell(true)
				end, { silent = true, buffer = true, desc = "Molten: run cell and move to next" })
				vim.keymap.set("n", "<A-r><A-b>", function()
					run_cell(true)
				end, { silent = true, buffer = true, desc = "Molten: run cell and move to next" })
				vim.keymap.set("n", "<A-r><A-g>", function()
					run_cell(false)
				end, { silent = true, buffer = true, desc = "Molten: run cell" })
		...
		vim.api.nvim_create_autocmd("BufEnter", {
			pattern = "*.py",
			callback = function(e)
				if python_is_active() == false then
					return
				end
		...
		vim.api.nvim_create_autocmd("BufEnter", {
			...
			callback = function(e)
				...
				local function smart_down()
					...
					if
						vim.fn.searchpair("^" .. tags.cell_start .. "$", "", "^" .. tags.cell_end .. "$", "ncbW") > 0
						and next_text ~= tags.cell_end
					then
						return vim.cmd("normal! " .. "j")
					end
				...
				vim.keymap.set("n", "<A-n>", function()
					...
					if
						is_empty == false
						and (is_markdown or (next_block_end == vim.fn.line("$") and vim.fn.getline("$") ~= ""))
					then
						table.insert(lines, "")
					end
				...
				end, { buffer = true, desc = "Insert new Python code block after current" })
				...
				end, { buffer = true, desc = "Insert new Python code block before current" })

				-- Split block
				vim.keymap.set("n", "<A-s><A-s>", function()
					...
					if
						...
						and end_below > start_above
						and cursor_pos > start_above
						and cursor_pos < end_below
					then
						-- Don't split if block only contains 1 line
						-- Don't split if cursor is on last line of block
						if end_below - start_above == 2 then
							vim.notify(
								"Cannot split: code block must have at least 2 lines.",
								vim.log.levels.WARN,
								{ timeout = 3000 }
							)
						elseif end_below - cursor_pos == 1 then
							vim.notify(
								"Cannot split: cursor is at the end of the code block.",
								vim.log.levels.WARN,
								{ timeout = 3000 }
							)
						else
							vim.api.nvim_buf_set_lines(
								...
								cursor_pos,
								false,
								{ tags.cell_end, "", tags.cell_start }
							)
						end
					else
						vim.notify(
							"Cannot split: cursor is not inside a code block.",
							vim.log.levels.WARN,
							{ timeout = 3000 }
						)
					end
				end, { buffer = true, desc = "Split code block at cursor position" })

				-- Delete block and move to prior
				vim.keymap.set("n", "<A-d><A-d>", function()
				...
				end, { buffer = true, desc = "Delete code block and jump to previous" })
			end,
		})
	end,
