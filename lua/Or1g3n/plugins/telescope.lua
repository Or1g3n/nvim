return {
	...
	config = function()
		...
		telescope.setup({
			...
			defaults = {
				...
				file_ignore_patterns = {
					...
					"^node_modules[\\/]",
					"^vendor[\\/]", -- Node.js and PHP dependencies
					...
					"^%.idea[\\/]",
					"^%.vscode[\\/]",
					"^%.nuget[\\/]", -- IDE settings (IntelliJ, VSCode)
					"%.swp$",
					"%.swo$", -- Vim swap files
		...
		map.set(
			"n",
			"<leader>fc",
			':lua require("telescope.builtin").find_files({ cwd= vim.fn.stdpath("config"), prompt_title = "Find Config Files" })<CR>',
			{ silent = true, desc = "Telescope: Fuzzy find config files" }
		)
		map.set(
			"n",
			"<leader>fgc",
			":Telescope grep_string<CR>",
			{ desc = "Telescope: Fuzzy find string under cursor in cwd" }
		)
		...
		map.set(
			"n",
			"<leader>fr",
			":lua require('telescope.builtin').oldfiles({ prompt_title = 'Recent Files' })<CR>",
			{ desc = "Telescope: Fuzzy find recent files" }
		)
		map.set(
			"n",
			"<leader>fsp",
			":Telescope spell_suggest<CR>",
			{ desc = "Telescope: Fuzzy find spell suggestions" }
		)
